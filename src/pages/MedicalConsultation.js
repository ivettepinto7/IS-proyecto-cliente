...
import { faCapsules, faPenToSquare, faPlusCircle, faTrashAlt } from "@fortawesome/free-solid-svg-icons";
                  ...
                  <div className="lg:grid lg:grid-cols-2 md:grid md:grid-cols-2 m-6">
                    <div className="mx-2 text-center border-2 rounded-lg py-4 my-2 max-h-100">
                      <div className="flex justify-center items-center">
                        <FontAwesomeIcon className="text-blue-800 text-4xl mx-4" icon={faCapsules} />
                        <h2 className="lg:text-2xl md:text-xl sm:text-xs xsm:text-xs"><b>Agregar medicamento</b></h2>
                      </div>
                      <div>
                        {values.list && values.list.length > 0 ? (
                          values.list.map((item, index) => (
                            <div className="flex justify-center items-center" key={index} >
                              ...
                              <ErrorMessage className="text-xs bg-red-600" name={`list.${index}.medicine`} />
                              <Field as={CustomInput} placeholder="Dosis" min={1} className="bg-gray-700 text-white text-sm w-1/5 mx-2" name={`list.${index}.doses`} key={`list.${index}.doses`} />
                              <ErrorMessage className="text-xs bg-red-600" name={`list.${index}.doses`} />
                              <Field as={CustomInput} placeholder="Cantidad" min={1} className="bg-gray-700 text-white text-sm w-1/5 mx-2" name={`list.${index}.quantity`} key={`list.${index}.quantity`} />
                              ...
                              >
                                <FontAwesomeIcon className="text-blue-800 text-xl" icon={faPlusCircle} />
                              </button>
                              <button
                                className="mx-2"
                                type="button"
                              ...
                              >
                                <FontAwesomeIcon className="text-red-600 text-xl" icon={faTrashAlt} />
                              </button>
                            </div>
                          ))
                        ) : (
                          <button className="text-sm underline" type="button" onClick={() => arrayHelpers.push('')}>
                            Agregar
                          </button>
                        )}
                      </div>
